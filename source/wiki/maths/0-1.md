...
若极限存在，那么是**可去间断点**。如果左右极限不相等，那么是**跳跃间断点**，左右极限之差为**跃度**。若是震荡的（如 $f(x)=\sin \frac 1 x$），则是**震荡间断点**，若左或右极限为无穷，则是**无穷间断点**。若 $f(x)$ 在 $[a,b]$ 连续，$(a,b)$ 可导，则 $f'(x)$ 只可能存在震荡间断点[^1]。
...
{% box %}
4. 计算 $\displaystyle\int \sec^3 \d x$。

令 $u=\tan x,v=\sec x$，则化为 $\tan x\sec x-\displaystyle\int \sec x \tan^2 x \d x$，第二坨等于 $\displaystyle\int \sec x(\sec^2 x-1)\d x$，于是有：

$$
\int \sec^3 \d x=\tan x\sec x-\int\sec^3 x\d x+\int \sec x\d x\\
\int \sec^3 \d x=\tan x\sec x-\int\sec^3 x\d x+\ln|\sec x+\tan x|\\
\int \sec^3 \d x=\frac 1 2 (\tan x\sec x+\ln|\sec x+\tan x|)+C
$$
{% endbox %}
...
积分上限函数 $\Phi(x)=\displaystyle\int_{a}^x f(t)\d t$。若 $f(x)$ 在 $[a,b]$ 连续，那么 $\Phi(x)$ 在 $[a,b]$ 上可导，且 $\Phi'(x)=f(x)$。注意在求解积分上限函数时，一定要保证被积函数和上限无关，如果有，一定要转化。
...
6. D，45 直接让数列正负交替
<!--folder 填空题-->
1. $6$，套路地将改写为 $\frac{e^{\ln(1+ax^2)\sin x}-1}{x^3}$，分子的指数是 $0$，等价无穷小成 $\ln(1+ax^2)\sin x$，然后怎么搞都行。
2. $\left(x-\frac 1 2\right)^2 +y^2=\frac 1 4$，由于算出来个除以 $0$，因此把 $x,y$ 坐标互换再算。
3. $y=x-1$，略。
4. $31e$，莱布尼茨公式展开。
5. $\arctan e^x + C$，略。
6. $\sin x^2$，令 $u=t-x$ 即可。
<!--folder 三-->
洛三次得到 $-2$。
<!--folder 四-->
$2e^2$，直接隐函数求导两次，没了。
...
<!--folder 九-->
{% mark color:red 我能在中值定理证明的拦截下通过微积分 A1 吗 %}
{% endfolders %}

### 5.3 吉林大学 微积分A1 2023 期末

<iframe src="/wiki/maths/jlucal2023.pdf" height="500px" width="100%"></iframe>
...
3. A，洛完之后震荡。
4. D，略。
5. A，注意内层函数不能取到常数 $u_0$，所以 BD 不一定对。
6. C，第二个极限可以未定义，第三个可以有跳跃间断点，第四个让其震荡到无穷即可，第五个随便搞一个跳跃的函数就炸了。官方答案认为第一个也是错的，我觉得这里不能是 $x=y^2$ 这种。
<!--folder 填空题-->
1. $2\ln 2-1$，略。
2. $2f'(x_0)$，略。
3. $\sqrt{1+x^2}\d x$，略。
4. $a$，略。
5. $1$，略。
6. $\frac{5\pi}{16}$，这里比较好用的思路是暴力降次，然后利用对称性能消掉大部分的项。
<!--folder 三-->
$3$，略。
<!--folder 四-->
...
取对数后直接泰勒展开，$e^{1/2}$。
<!--folder 九-->
{% mark color:red 我怎么这么菜。 %}
{% endfolders %}

[^1]: 可以参考 https://zhuanlan.zhihu.com/p/651443880。