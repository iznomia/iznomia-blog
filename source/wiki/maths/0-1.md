...
$f(x)$ 在 $I$ 内的全体原函数称为 $f(x)$ 在 $I$ 内的不定积分，记作 $\displaystyle\int f(x)\d x$。

### 3.1 第一换元法（凑微分法）

如果 $\displaystyle\int g(x)\d x$ 是难求的，但是 $\displaystyle\int f(u)\d u$ 是好求的，其中 $g(x)=f(\varphi(x))\varphi'(x)$，此时令 $u=\varphi(x)$，那么 $$
