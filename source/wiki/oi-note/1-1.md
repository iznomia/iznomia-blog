...
[Portal](https://www.luogu.com.cn/problem/P3295).

并查集？复杂度不对，考虑倍增并查集。使用类似于 ST 表的结构，操作时直接操作高层节点的并查集，处理完之后将高层内容下放，这样可以得到完整的答案。[代码](https://loj.ac/s/1889627)。

#### [USACO23JAN P] Tractor Paths

[Portal](https://www.luogu.com.cn/problem/P9019).

最短距离能走到更远肯定是更好的，因此第一问直接倍增。

第二问要求 $dis(a,u)+dis(u,b)=dis(a,b)$ 的 $u$ 的个数，假设从 $a$ 向右跳 $i$ 步可以到达 $r_i$，从 $b$ 向左跳 $ans-i$ 步可以到达 $l_i$，那么必定 $l_i\le r_i$（否则你是怎么从 $a$ 走到 $b$ 的）。实际上画个图可以发现，$l_1\le r_1\le l_2\le r_2\cdots$，因此就是若干个不相交的区间来贡献答案。

![](https://pic1.imgdb.cn/item/69512397a0c391c56de5e612.png)

注意至少跳一步，否则会涉及到和 $a,b$ 取 max、min 的问题。[代码](https://qoj.ac/submission/1876652)。
